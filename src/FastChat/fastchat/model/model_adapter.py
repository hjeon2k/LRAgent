...
peft_model_cache = {}
SEED=42
def monkey_patch_peftmodel():
    PeftModel.past_key_values = {}  # modified for LRAgent
    PeftModel.past_lr_caches = {}  # modified for LRAgent
    PeftModel.cached = False
    PeftModel.num_ex = 0
    torch.manual_seed(SEED)
    torch.cuda.manual_seed(SEED)

def share_kvcache(model: torch.nn.Module, model_path: str):
    targets = model.peft_config[model_path].target_modules
    for name, module in model.base_model.named_modules():
        name_ = name.split('.')[-1]
        if name_ in targets and name_ in ["k_proj", "v_proj"]:
            setattr(module, "lrcache", True)
            # pass
...
class PeftModelAdapter:
    ...
    def load_model(self, model_path: str, from_pretrained_kwargs: dict):
        ...
        if peft_share_base_weights:
            ...
            share_kvcache(model, model_path)
            return model, tokenizer
