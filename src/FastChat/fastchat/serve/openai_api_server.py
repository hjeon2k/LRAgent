...
@app.post("/v1/clear_kvcache", dependencies=[Depends(check_api_key)])
async def clear_kvcache(request: APIClearKVCache):
    worker_addr = await get_worker_address(request.model)
    await fetch_remote(worker_addr + "/model_clear_kvcache",
                       {"model" : request.model})
