...
class LRCache(Cache):
    """
    A low rank cache that grows dynamically as more tokens are generated. This is the default for generative models.

    It stores the low rank Value states as a list of tensors, one for each layer. The expected shape for each tensor is
    ...
    def __iter__(self):
        for layer_idx in range(len(self)):
            yield self.value_cache[layer_idx]
    ...
    def update(
        ...
        if value_states is not None:
            if len(self.value_cache) <= layer_idx:
                ...
                for _ in range(len(self.value_cache), layer_idx):
                    self.value_cache.append([])
                self.value_cache.append(value_states)
            elif (
                len(self.value_cache[layer_idx]) == 0
    ...
    def get_seq_length(self, layer_idx: Optional[int] = 0) -> int:
        ...
        # TODO: deprecate this function in favor of `cache_position`
        is_empty_layer = (
            len(self.value_cache) == 0  # no cache in any layer
            or len(self.value_cache) <= layer_idx  # skipped `layer_idx` and hasn't run a layer with cache after it
            or len(self.value_cache[layer_idx]) == 0  # the layer has no cache
        )
        layer_seq_length = self.value_cache[layer_idx].shape[-2] if not is_empty_layer else 0
    ...
    def to_legacy_cache(self) -> Tuple[Tuple[torch.Tensor], Tuple[torch.Tensor]]:
        ...
        legacy_cache = ()
        for layer_idx in range(len(self)):
            legacy_cache += (self.value_cache[layer_idx])
        return legacy_cache

    @classmethod
    def from_legacy_cache(cls, past_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None) -> "DynamicCache":
        """Converts a cache in the legacy cache format into an equivalent `DynamicCache`. Used for
        backward compatibility."""
        cache = cls()
        if past_values is not None:
            for layer_idx in range(len(past_values)):
                value_states = past_values[layer_idx]
                cache.update(value_states, layer_idx)
        return cache
