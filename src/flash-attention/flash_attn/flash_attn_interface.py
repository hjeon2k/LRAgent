...
if USE_TRITON_ROCM:
    ...
    import flash_lora_attn_2_cuda as flash_lora_attn_gpu  # TODO: install flash_lora_attn_2_cuda and import as flash_lora_attn_gpu
...
@_torch_custom_op_wrapper("flash_attn::_flash_lora_attn_forward", mutates_args=(), device_types="cuda")
def _flash_lora_attn_forward(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    lr_v: torch.Tensor,
    ...
    q, k, v, lr_v, lr_B = [maybe_contiguous(x) for x in (q, k, v, lr_v, lr_B)]
    out, softmax_lse, S_dmask, rng_state = flash_lora_attn_gpu.fwd(
        q,
        k,
        v,
        lr_v,
...
_wrapped_flash_lora_attn_forward = _flash_lora_attn_forward
...
class FlashLoraAttnFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q,
        ...
        if head_size_og % 8 != 0:
            ...
            k = torch.nn.functional.pad(k, [0, 8 - head_size_og % 8])
            v = torch.nn.functional.pad(v, [0, 8 - head_size_og % 8])
        out_padded, softmax_lse, S_dmask, rng_state = _wrapped_flash_lora_attn_forward(
            q,
            k,
            v,
            lr_v,
...
def flash_lora_attn_func(
    ...
    Arguments:
        ...
        k: (batch_size, seqlen, nheads_k, headdim)
        v: (batch_size, seqlen, nheads_k, headdim)
        lr_v: (batch_size, seqlen, r)
        lr_B: (r, nheads_k, headdim)
           (they might not have the right scaling).
    Return:
        out: (batch_size, seqlen, nheads, headdim).
        softmax_lse [optional, if return_attn_probs=True]: (batch_size, nheads, seqlen). The
        ...
        S_dmask [optional, if return_attn_probs=True]: (batch_size, nheads, seqlen, seqlen).
            ...
            pattern (negative means that location was dropped, nonnegative means it was kept).
    """
    return FlashLoraAttnFunc.apply(
        q,
        k,
        v,
        lr_v,
