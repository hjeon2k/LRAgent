...
void set_params_fprop(Flash_fwd_params &params,
                      ...
                      const size_t lora_dim,
                      // device pointers
                      const at::Tensor q,
                      const at::Tensor k,
                      const at::Tensor v,
                      const at::Tensor lr_v,
                      const at::Tensor lr_b,
                      at::Tensor out,
                      void *cu_seqlens_q_d,
                      void *cu_seqlens_k_d,
                      void *seqused_k,
    ...
    params.lora_v_ptr = lr_v.data_ptr();
    params.lora_b_ptr = lr_b.data_ptr();
    ...
    params.lora_v_row_stride = lr_v.stride(-2);
    params.lora_b_head_stride = lr_b.stride(0);
    params.lora_b_row_stride = lr_b.stride(1);
    ...
    if (cu_seqlens_q_d == nullptr) {
        ...
        params.lora_v_batch_stride = lr_v.stride(0);
    ...
    params.lora_dim = lora_dim;
    params.total_q = b * seqlen_q;  // For non-varlen mode
...
/* === COMMENTED OUT: Only mha_fwd is needed for LoRA attention ===
...
=== END COMMENTED OUT: set_params_dgrad === */
...
std::tuple<at::Tensor, at::Tensor> set_params_splitkv(Flash_fwd_params &params, const int batch_size,
    ...
    const int block_n = head_size <= 64 ? 256 : (head_size <= 128 ? 64 : 64);
...
mha_fwd(at::Tensor &q,         // batch_size x seqlen_q x num_heads x round_multiple(head_size, 8)
        ...
        const at::Tensor &lr_v,    // LoRA V tensor, batch_size x seqlen_k x lora_dim(== 8)
        const at::Tensor &lr_b,    // LoRA B tensor, num_heads_k x lora_dim x head_size let all batches has the same adapter.
    ...
    TORCH_CHECK(lr_v.dtype() == q_dtype, "LoRA V tensor must have the same dtype as query");
    TORCH_CHECK(lr_b.dtype() == q_dtype, "LoRA B tensor must have the same dtype as query");
    ...
    TORCH_CHECK(lr_v.stride(-1) == 1, "LoRA V tensor must have contiguous last dimension");
    TORCH_CHECK(lr_b.stride(-1) == 1, "LoRA B tensor must have contiguous last dimension");
    ...
    const int lora_dim = lr_v.size(-1);
    TORCH_CHECK(batch_size > 0, "batch size must be positive");
    TORCH_CHECK(lora_dim == 8, "Currently only lora_dim=8 is supported. Got lora_dim=" + std::to_string(lora_dim));
    TORCH_CHECK(head_size <= 128, "FlashLoRAAttention forward only supports head dimension at most 128");
    ...
    CHECK_SHAPE(lr_v, batch_size, seqlen_k, lora_dim);
    CHECK_SHAPE(lr_b, num_heads_k, lora_dim, head_size);
...
mha_varlen_fwd(at::Tensor &q,  // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
               ...
               const at::Tensor &lr_v,    // LoRA V tensor, total_k x num_heads_k x lora_dim or num_blocks x page_block_size x num_heads_k x lora_dim if there's a block_table.
               const at::Tensor &lr_b,    // LoRA B tensor, num_heads_k x lora_dim x head_size let all batches has the same adapter.
    ...
    const int lora_dim = lr_v.size(-1);
    TORCH_CHECK(lr_b.size(1) == lora_dim, "LoRA B tensor has incorrect shape");
    TORCH_CHECK(lr_b.size(2) == head_size, "LoRA B tensor has incorrect shape");
    ...
    //FIX ME: Do I need to round lora_dim too?
...
mha_bwd(const at::Tensor &dout,  // batch_size x seqlen_q x num_heads, x multiple_of(head_size_og, 8)
        ...
        const at::Tensor &lr_v,    // LoRA V tensor
        const at::Tensor &lr_b,    // LoRA B tensor
...
=== END COMMENTED OUT: mha_varlen_fwd, run_mha_bwd, mha_bwd, mha_varlen_bwd, mha_fwd_kvcache === */
} // namespace FLASH_NAMESPACE

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.doc() = "FlashLoRAAttention";
