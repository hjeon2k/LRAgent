...
// Variant of gemm_rs where tCrB has transposed layout: (MMA, MMA_K, MMA_N) instead of (MMA, MMA_N, MMA_K)
// This is useful when B matrix is stored in K×N format (e.g., LoRA lr_b matrix)
template<typename Tensor0, typename Tensor1, typename Tensor2, typename Tensor3,
         typename TiledMma, typename TiledCopy, typename ThrCopy>
__forceinline__ __device__ void gemm_rs_transpose(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const& tCsB,
                                          TiledMma tiled_mma, TiledCopy smem_tiled_copy_B,
                                          ThrCopy smem_thr_copy_B) {
    CUTE_STATIC_ASSERT_V(size<1>(tCrA) == size<1>(acc));                     // MMA_M
    CUTE_STATIC_ASSERT_V(size<2>(tCrB) == size<2>(acc));                     // MMA_N (transposed: use size<2> instead of size<1>)
    CUTE_STATIC_ASSERT_V(size<2>(tCrA) == size<1>(tCrB));                     // MMA_K (transposed: use size<1> instead of size<2>)
    Tensor tCrB_copy_view = smem_thr_copy_B.retile_D(tCrB);
    CUTE_STATIC_ASSERT_V(size<1>(tCsB) == size<1>(tCrB_copy_view));            // K (first dimension in transposed layout)
    cute::copy(smem_tiled_copy_B, tCsB(_, _, _0{}), tCrB_copy_view(_, _, _0{}));
    #pragma unroll
    for (int i = 0; i < size<2>(tCrA); ++i) {
        if (i < size<2>(tCrA) - 1) {
        ...
        // tCrB indexing: (_, i, _) to get K-th slice from (MMA, MMA_K, MMA_N)
        cute::gemm(tiled_mma, tCrA(_, _, i), tCrB(_, i, _), acc);
    }
}
