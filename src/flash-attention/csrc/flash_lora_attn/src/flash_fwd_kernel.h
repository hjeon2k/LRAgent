...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    constexpr int kLoRADim = Kernel_traits::kLoRADim;
    ...
    Tensor mlrV = make_tensor(make_gmem_ptr(reinterpret_cast<Element*>(params.lora_v_ptr)
                                          + binfo.k_offset(params.lora_v_batch_stride, params.lora_v_row_stride, bidb)),
                            make_shape(binfo.actual_seqlen_k, kLoRADim),
                            make_stride(params.lora_v_row_stride, _1{}));
    Tensor glrV = local_tile(mlrV(_, _), Shape<Int<kBlockN>, Int<kLoRADim>>{},
                           make_coord(_, 0));  // (kBlockN, kLoRADim, nblocksN)
    Tensor mlrB = make_tensor(make_gmem_ptr(reinterpret_cast<Element*>(params.lora_b_ptr)),
                            make_shape(params.h_k, kLoRADim, params.d),
                            make_stride(params.lora_b_head_stride, params.lora_b_row_stride, _1{}));
    Tensor glrB = local_tile(mlrB(bidh / params.h_h_k_ratio, _, _), Shape<Int<kLoRADim>, Int<kHeadDim>>{},
                           make_coord(0, 0));  // (kLoRADim, kHeadDim)
    ...
    Tensor slrV = make_tensor(sV.data() + size(sV), typename Kernel_traits::SmemLayoutlrV{});
    Tensor slrVt = make_tensor(slrV.data(), typename Kernel_traits::SmemLayoutlrVtransposed{});
    Tensor slrVtNoSwizzle = make_tensor(slrV.data().get(), typename Kernel_traits::SmemLayoutlrVtransposedNoSwizzle{});
    
    Tensor slrB = make_tensor(make_smem_ptr(reinterpret_cast<Element *>(smem_)), typename Kernel_traits::SmemLayoutlrB{});
    Tensor slrBt = make_tensor(slrB.data(), typename Kernel_traits::SmemLayoutlrBtransposed{});
    Tensor slrBtNoSwizzle = make_tensor(slrB.data().get(), typename Kernel_traits::SmemLayoutlrBtransposedNoSwizzle{});
    ...
    // For LoRA V copy: only kBlockN threads participate (when kNThreads > kBlockN)
    // Use tidx % kBlockN to get valid thread slice, and guard copy with tidx < kBlockN
    typename Kernel_traits::GmemTiledCopylrV gmem_tiled_copy_lrV;
    const int lrV_tidx = tidx;
    auto gmem_thr_copy_lrV = gmem_tiled_copy_lrV.get_thread_slice(lrV_tidx);
    Tensor tlrVglrV = gmem_thr_copy_lrV.partition_S(glrV);  // (lrVCPY, lrVCPY_N, lrVCPY_K, nblocksN)
    Tensor tlrVslrV = gmem_thr_copy_lrV.partition_D(slrV);

    typename Kernel_traits::GmemTiledCopylrB gmem_tiled_copy_lrB;
    const int lrB_tidx = tidx;
    auto gmem_thr_copy_lrB = gmem_tiled_copy_lrB.get_thread_slice(lrB_tidx);
    Tensor tlrBglrB = gmem_thr_copy_lrB.partition_S(glrB);  // (lrBCPY, lrBCPY_N, lrBCPY_K)
    Tensor tlrBslrB = gmem_thr_copy_lrB.partition_D(slrB);
    ...
    typename Kernel_traits::TiledMma_lrV tiled_mma_lrV;
    auto thr_mma_lrV = tiled_mma_lrV.get_thread_slice(tidx);
    Tensor tLOrlrVt = thr_mma_lrV.partition_fragment_B(slrVtNoSwizzle);
    Tensor tLOrLO = partition_fragment_C(tiled_mma_lrV, Shape<Int<kBlockM>, Int<kLoRADim>>{});  // MMA, MMA_M, MMA_LoRADim(K)

    typename Kernel_traits::TiledMma_lrB tiled_mma_lrB;
    auto thr_mma_lrB = tiled_mma_lrB.get_thread_slice(tidx);
    Tensor tOrlrBt = thr_mma_lrB.partition_fragment_B(slrBtNoSwizzle);
    ...
    auto smem_tiled_copy_lrV = make_tiled_copy_B(typename Kernel_traits::SmemCopyAtomLoRA{}, tiled_mma_lrV);
    auto smem_thr_copy_lrV = smem_tiled_copy_lrV.get_thread_slice(tidx);
    Tensor tLOslrVt = smem_thr_copy_lrV.partition_S(slrVt);

    auto smem_tiled_copy_lrB = make_tiled_copy_B(typename Kernel_traits::SmemCopyAtomLoRA{}, tiled_mma_lrB);
    auto smem_thr_copy_lrB = smem_tiled_copy_lrB.get_thread_slice(tidx);
    Tensor tOslrBt = smem_thr_copy_lrB.partition_S(slrBt);
    ...
    Tensor clrV = make_identity_tensor(make_shape(size<0>(slrV), size<1>(slrV)));
    Tensor clrB = make_identity_tensor(make_shape(size<0>(slrB), size<1>(slrB)));
    ...
    Tensor tlrVclrV = gmem_thr_copy_lrV.partition_S(clrV);
    Tensor tlrBclrB = gmem_thr_copy_lrB.partition_S(clrB);

    // Allocate predicate tensors for k
    Tensor tQpQ = make_tensor<bool>(make_shape(size<2>(tQsQ)));
    Tensor tKVpKV = make_tensor<bool>(make_shape(size<2>(tKsK)));
    Tensor tlrVplrV = make_tensor<bool>(make_shape(size<2>(tlrVslrV)));
    Tensor tlrBplrB = make_tensor<bool>(make_shape(size<2>(tlrBslrB)));
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            ...
            // LoRA V copy: only first kBlockN threads participate (when kNThreads > kBlockN)
            if (lrV_tidx < Kernel_traits::kNThreadsPerLoadLoRAV){
                FLASH_NAMESPACE::copy</*Is_even_MN=*/true, true>(gmem_tiled_copy_lrV, tlrVglrV(_, _, _, n_block), tlrVslrV, tlrVclrV, tlrVplrV);
            }
        } else {
            ...
            if (lrV_tidx < Kernel_traits::kNThreadsPerLoadLoRAV){
                FLASH_NAMESPACE::copy<Is_even_MN, true, /*Clear_OOB_MN=*/true>(
                    gmem_tiled_copy_lrV, tlrVglrV(_, _, _, n_block), tlrVslrV, tlrVclrV, tlrVplrV, binfo.actual_seqlen_k - n_block * kBlockN
                );
            }       
        }
        cute::cp_async_fence();
        ...
        if (n_block > n_block_min) {
            ...
            if (lrB_tidx < Kernel_traits::kNThreadsPerLoadLoRAB){
                FLASH_NAMESPACE::copy<true, true>(gmem_tiled_copy_lrB, tlrBglrB(_, _, _), tlrBslrB, tlrBclrB, tlrBplrB);
            }
            cute::cp_async_fence();
        }

        // TODO: when we have key_padding_mask we'll need to Check_inf
        // LoRA: Use dual version to update both acc_o and tLOrLO with a single softmax computation
        masking_step == 0
            ? softmax.template softmax_rescale_o_dual</*Is_first=*/true,  /*Check_inf=*/Is_causal || Is_local>(acc_s, acc_o, tLOrLO, params.scale_softmax_log2)
            : softmax.template softmax_rescale_o_dual</*Is_first=*/false, /*Check_inf=*/Is_causal || Is_local>(acc_s, acc_o, tLOrLO, params.scale_softmax_log2);
        ...
        FLASH_NAMESPACE::gemm_rs(tLOrLO, tOrP, tLOrlrVt, tLOslrVt, tiled_mma_lrV, smem_tiled_copy_lrV, smem_thr_copy_lrV);
    ...
    for (; n_block >= n_block_min; --n_block) {
        ...
        __syncthreads();
        FLASH_NAMESPACE::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, tVgV(_, _, _, n_block), tVsV, tKVcKV, tKVpKV);
        // LoRA V copy: only first kBlockN threads participate
        if (lrV_tidx < Kernel_traits::kNThreadsPerLoadLoRAV){
            FLASH_NAMESPACE::copy</*Is_even_MN=*/true, true>(gmem_tiled_copy_lrV, tlrVglrV(_, _, _, n_block), tlrVslrV, tlrVclrV, tlrVplrV);
        }
        cute::cp_async_fence();
        ...
        // LoRA: Use dual version to update both acc_o and tLOrLO with a single softmax computation
        softmax.template softmax_rescale_o_dual</*Is_first=*/false, /*Check_inf=*/Is_local>(acc_s, acc_o, tLOrLO, params.scale_softmax_log2);
        ...
        // if using m16n8k16 or (4, MMA_M, MMA_N) if using m16n8k8.
        Tensor tOrP = make_tensor(rP.data(), FLASH_NAMESPACE::convert_layout_acc_Aregs<Kernel_traits::TiledMma>(rP.layout()));
        Tensor tLOrP = make_tensor(rP.data(), FLASH_NAMESPACE::convert_layout_acc_Aregs<Kernel_traits::TiledMma_lrV>(rP.layout()));
        FLASH_NAMESPACE::gemm_rs(acc_o, tOrP, tOrVt, tOsVt, tiled_mma, smem_tiled_copy_V, smem_thr_copy_V);
        FLASH_NAMESPACE::gemm_rs(tLOrLO, tLOrP, tLOrlrVt, tLOslrVt, tiled_mma_lrV, smem_tiled_copy_lrV, smem_thr_copy_lrV);
    }
    ...
    Tensor lse = softmax.template normalize_softmax_lse_dual<Is_dropout>(acc_o, tLOrLO, params.scale_softmax, params.rp_dropout);
    
    // LoRA: Now compute LO @ lr_B and add to acc_o (after normalization)
    Tensor rLO = FLASH_NAMESPACE::convert_type<Element>(tLOrLO);
    FLASH_NAMESPACE::gemm_rs(
            acc_o, rLO, tOrlrBt, tOslrBt, tiled_mma_lrB, smem_tiled_copy_lrB, smem_thr_copy_lrB);
...
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    const index_t row_offset_v = block_table == nullptr
        ...
        //FIX ME : No block table for LoRA cache
    const index_t row_offset_lrV = 
        binfo.k_offset(params.lora_v_batch_stride, params.lora_v_row_stride, bidb_cache);
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        // LoRA: Use dual version to update both acc_o and tLOrLO with a single softmax computation
        masking_step == 0
            ? softmax.template softmax_rescale_o_dual</*Is_first=*/true,  /*Check_inf=*/Is_causal || Is_local || !Is_even_MN>(acc_s, acc_o, tLOrLO, params.scale_softmax_log2)
            : softmax.template softmax_rescale_o_dual</*Is_first=*/false, /*Check_inf=*/Is_causal || Is_local || !Is_even_MN>(acc_s, acc_o, tLOrLO, params.scale_softmax_log2);
