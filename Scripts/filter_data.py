...
with open(args.source_path,"r") as f:
    ...
    for item in lines:
        ...
        plan_data={"input":systemprompt+f"\nQuestion:{question}\nThought: {response_keyword}","output":""}
        ...
        for index,(a,t,o) in enumerate(zip(data["actions"],data["thoughts"],data["observations"])):
            ...
            if '[' in a and ']' in a:
                ...
                keyword=a[a.find('[')+1:a.find(']')]
                if action_type == 'Reflect' :
                    action_data["input"]+=f"Thought: {response_keyword}"
                    ...
                    action_data["input"]+=f"Thought: {response_keyword}"+t+"\n"
                    action_data["input"]+=f"Aciton: "+action_type+f"[{response_keyword}"
                ...
                action_data["input"]+=f"Thought: {response_keyword}"+t+"\n"
                action_data["input"]+=f"Action: {response_keyword}"+a+"\n"
                action_data["input"]+=f"Obversation: "+o+"\n"
