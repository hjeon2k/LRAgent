...
from benchmark_run.utils import call_web_search, parse_result
...
class BaseAgent:
    ...
    def step(self) -> None:
        ...
        if action_type == 'Retrieve':
            ...
            try:
                responses = call_web_search("https://api.bing.microsoft.com/v7.0/search","5fc96b4311e74e519dae3c2acfa56ef8",argument,count=1)
...
class MetaZeroshotThinkAgentHotPotQA(BaseAgent):
    ...
    def prompt_agent(self,agent_type) -> str:
        ...
        if agent_type == 'plan':
            llm_target = self.llm_plan
        elif agent_type == 'action':
            llm_target = self.llm_action
        elif agent_type == 'reflect':
            llm_target = self.llm_reflect
        generation, t, s = llm_target(prompt)
        self.check_run_error(generation)
        self.profile_t.append([t, agent_type])
        self.profile_s.append([s['prompt_tokens'], s['total_tokens'], s['completion_tokens'], agent_type])
        return generation  # Removed format_step(generation) for LRAgent
    ...
    def _think(self):
        self.scratchpad += f'\nThought: {self.response_keyword}'
        if self.last_step == "Finish":
            thought = self.prompt_agent(agent_type='reflect')
        else:
            thought = self.prompt_agent(agent_type="plan")
        self.scratchpad += thought
        print(self.scratchpad.replace(self.response_keyword, "").split('\n')[-1])

    def _action(self):
        self.scratchpad += f'\nAction: {self.response_keyword}'
        if self.last_step == "Finish":
            ...
            action_type = self.prompt_agent(agent_type='plan')
            self.scratchpad += f'{action_type}[{self.response_keyword}' # Modified for LRAgent
            action_argument = self.prompt_agent(agent_type='action')
            self.scratchpad += f'{action_argument}' if action_argument.find(']') > 0 else f'{action_argument}]'  # Modified for LRAgent
            action_argument = action_argument[:action_argument.find(']')] if action_argument.find(']') > 0 else action_argument
            print(self.scratchpad.replace(self.response_keyword, "").split('\n')[-1])
            return action_type, action_argument
    ...
    def step(self) -> None:
        ...
        self.scratchpad += f'\nObservation: '
        if action_type == 'Finish':
            if self.answer != '':  # mostly not likely to be happen
                self.answer = argument
                if self.is_correct():
                    self.scratchpad += 'Answer is CORRECT'
                else:
            ...
            try:
                responses = call_web_search(query=argument)
    ...
    def _build_agent_prompt(self) -> str:
        self.tools = ""
        with open("Self_Plan/Tool_Selection/HotpotQA_Tools.json") as f:
...
class MetaZeroshotThinkAgentScienceQA(BaseAgent):
    ...
    def reward(self) -> float:
        ...
        return [1.0 if prediction.strip().lower() == self.key.strip().lower() else 0.0]
    ...
    def step(self) -> None:
        ...
        if action_type == 'Finish':
            ...
            self.scratchpad += 'Invalid Action. Valid Actions are Retrieve[<topic>] BingSearch[<topic>] Finish[<answer>] Image2Text[image] and Reflect[right/wrong].'
    ...
    def _build_agent_prompt(self) -> str:
        self.tools = ""
        with open("Self_Plan/Tool_Selection/ScienceQA_Tools.json") as f:
            tool_selected = json.load(f)
            for index,tool in enumerate(tool_selected):
                usage = tool["usage"]
                self.tools += f"({index+1}) {usage}\n"
